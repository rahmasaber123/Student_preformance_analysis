...
"""
"""Reusable UI cards: KPI tiles + page header with logo top-right."""
import base64
...
def render_page_header(title: str, subtitle: str = "") -> None:
    """Top bar on every authenticated page: title left, Kayfa logo right."""
    logo = _logo_b64()
    sub_html = f'<div class="kf-page-sub">{subtitle}</div>' if subtitle else ""
    html = f"""
    <div class="kf-page-header">
      <div class="kf-page-titles">
        <div class="kf-page-title">{title}</div>
        {sub_html}
      </div>
      <img class="kf-page-logo" src="data:image/png;base64,{logo}" alt="Kayfa"/>
    </div>
...
def render_kpi_row(items: list[dict]) -> None:
    """A row of KPI tiles. Each item: {label, value, hint?}"""
    cols = st.columns(len(items))
    for col, item in zip(cols, items):
        hint = f'<div class="kf-kpi-hint">{item.get("hint","")}</div>' if item.get("hint") else ""
        html = f"""
        <div class="kf-kpi">
          <div class="kf-kpi-label">{item['label']}</div>
          <div class="kf-kpi-value">{item['value']}</div>
          {hint}
        </div>
        """
...
def chart_hossam_pie(master: pd.DataFrame,
    ...
    # Compose a short two-line label that fits inside the slice
    inside_labels = [
        f"{name}<br>avg {grade:.1f}"
        for name, grade in zip(agg["course_name"], agg["avg_grade"])
    ]
    fig = go.Figure(go.Pie(
        labels=agg["course_name"],
        values=agg["students"],
        hole=0.45,
...
def chart_recursion_by_instructor() -> go.Figure:
    """Recursion failure rate AND absolute failed-student count by instructor.

    Combo chart: bars = failed students (left axis), line = failure rate (right
    axis). Values mirror the notebook's `recursion_rate` dataframe.
    """
    data = pd.DataFrame({
    ...
    fig.add_trace(go.Bar(
        x=data["instructor"], y=data["failed"],
    ...
    fig.add_trace(go.Scatter(
        x=data["instructor"], y=data["failure_rate"],
        mode="lines+markers+text",
        text=data["failure_rate"].apply(lambda x: f"{x}%"),
        textposition="top center",
        textfont=dict(size=13, color=BLUE[0]),
...
def chart_q9_engagement_trend(monthly: pd.DataFrame) -> go.Figure:
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=df["month"], y=df["events_per_student"],
        mode="lines+markers+text",
        line=dict(width=4, color=BLUE[2]),
        marker=dict(size=12, color=BLUE[2]),
        text=df["events_per_student"].round(1),
        textposition="top center",
